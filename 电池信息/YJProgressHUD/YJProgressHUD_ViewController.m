//
//  YJProgressHUD_ViewController.m
//  第三方相关
//
...
@interface YJProgressHUD_ViewController ()
@property (weak, nonatomic) IBOutlet UITextField *txfld;
@property (nonatomic,strong)MJProgressHUD *MJ_HUD;
@end

@implementation YJProgressHUD_ViewController{
...
- (void)viewDidLoad {
    ...
    //NetWekingHUD//----YJProgressHUD用法
//    [self NetWekingHUD];
    _MJ_HUD = [[MJProgressHUD alloc]init];
...
- (IBAction)SetString:(id)sender {
    [MJProgressHUD showAlertWithString:_txfld.text];
}
-(void)NetWekingHUD{
    
    UIView *test = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 400, 400)];
...
//            [YJProgressHUD showMessage:@"显示文字，1s隐藏" inView:self.view];
        ...
        case 1008:{
            //加载自定义图片，含文字
            [YJProgressHUD showMsgWithImage:@"很遗憾，失败了" imageName:@"tp.png" inview:self.view];
            break;
        }
        default:
            break;
